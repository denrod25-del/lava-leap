...
begin
  ...
  if p_board !~ '^(alltime|daily:\d{4}-\d{2}-\d{2})$' then return; end if;
  ...
  select count(*) into recent from public.scores
    where player_id = p_player_id and created_at > now() - interval '1 minute';
...
--  select public.submit_score('11111111-1111-1111-1111-111111111111','TestA','alltime',800,790,60000,0);    -- kept 1200 (max)
