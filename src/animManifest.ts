...
export interface AnimDef {
  ...
  frames: Array<string | number>; // texture keys for legacy chars OR numeric spritesheet frames
  frameRate: number;
  repeat: number;                 // -1 = loop
  sheetKey?: string;              // present when frames are indices inside a spritesheet
}
...
/** Production Climber spritesheet map. The atlas is a uniform 8-column grid of
 *  256×256 cells in this exact package order: idle, run, jump/fall, land,
...
const CLIMBER_STATE_DEFS: { state: PlayerState; frames: number[]; frameRate: number; repeat: number }[] = [
  { state: 'idle',        frames: [0, 1, 2, 3], frameRate: 6, repeat: -1 },
  { state: 'run',         frames: [4, 5, 6, 7, 8, 9, 10, 11], frameRate: 12, repeat: -1 },
 ...
 *  Climber uses numeric frames inside one uniform spritesheet. */
